...
# Plotting
ggplot(data = rs_plot, aes(x = VISITNUM, y = as.numeric(RSORRES), group = USUBJID)) +
  geom_line(color = "gray", size = 1) +  # Draw lines for trajectories
  geom_point(aes(color = ifelse(last_visit, "dropout", "active")), size = 2, shape = 21, fill = ifelse(rs_plot$last_visit, "red", "blue")) +  # Points colored based on condition
  scale_color_manual(values = c("active" = "blue", "dropout" = "red"),
                     labels = c("active" = "Active", "dropout" = "Dropout")) +  # Customizing colors and labels
...
# output summary
nsaa_total_gls <- gls(RSORRES ~ poly(VISIT, 2), 
                        data = rs_1005_nsaa, 
...
print(sig_values)
```
When the treatment effect is the same as the standard deviation (4)
