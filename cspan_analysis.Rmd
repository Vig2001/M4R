...
# cluster validation is a method which could be the same as that described above - look up the silhouette score!

# 3.) Ways to deal with different sizes of trajectories (because of dropout)
# simplest - just perform analysis on complete trajectories but only 3
# assign last value for the rest of time to complete incomplete trajectories
# assign average for the rest of time to complete incomplete trajectories

# 4.) Add grey bard to the bottom of the plot to show how many individuals at each time point
...
The summary indicates an AR(1) correlation structure in your GLS model, with a high phi estimate of 0.9639783, suggesting strong serial correlation.