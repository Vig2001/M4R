...
# Remove prefix and update names in cov_matrix_df
names(cov_matrix_df) <- gsub("NSAA1-", "", names(cov_matrix_df))
print(names(cov_matrix_df))
# Define the desired custom order (ensure these exactly match the actual variable names after prefix removal)
custom_order <- c("Walk", "Stand Up From Chair", "Run (10m)", "Stand", "Stand on Heels",
                  "Rise From Floor", "Jump", "Lifts Head", "Get to Sitting",
                  "Descend Box Step - Right", "Descend Box Step - Left",
                  "Climb Box Step - Right", "Climb Box Step - Left",
                  "Stand on One Leg - Right", "Stand on One Leg - Left",
                  "Hop Right Leg", "Hop Left Leg")

# Generate the order index for the dataframe
ordered_indices <- match(custom_order, names(cov_matrix_df))
cov_matrix_df <- cov_matrix_df[ordered_indices, ordered_indices]
# Reorder the covariance matrix based on these indices
rownames(cov_matrix_df) <- custom_order 
cov_matrix_df$variable <- rownames(cov_matrix_df)


# Melt the data frame for ggplot
long_cov_matrix <- pivot_longer(cov_matrix_df, cols = -variable, names_to = "variable2", values_to = "cov_value")

# Plot the heatmap
corrheatmap <- ggplot(data = long_cov_matrix, aes(x = variable, y = variable2, fill = cov_value)) +
  geom_tile() +
  scale_fill_gradient2(mid = "white", high = "red", low = "blue", midpoint = 0) +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, size = 10), 
        axis.text.y = element_text(angle = 0, size = 10)) +
  labs(x = "NSAA Item 1", y = "NSAA Item 2", fill = "Covariance")

ggsave("plots/corrheatmap.png", corrheatmap)
```
```{r}
# Mean Variance relationship

# Remove 'NSAA1-' from column names
colnames(X0) <- gsub("NSAA1-", "", colnames(X0))
print(colnames(X0))
# Create data frame with updated column names
feature_stats <- data.frame(
  feature = colnames(X0),
  mean = apply(X0, 2, mean),
  variance = apply(X0, 2, var)
)

# Create scatter plot
mvarplot <- ggplot(feature_stats, aes(x = mean, y = variance, label = feature)) +
  geom_point() +
  geom_label_repel(nudge_y = 0.05, box.padding = 0.35, point.padding = 0.5, segment.color = 'grey50') +  # Adjust label repel settings for better readability
  labs(x = "Mean", y = "Variance", title = "Mean-Variance Relationship of each NSAA Item") +
  theme_minimal()
ggsave("plots/mvarplot.png", mvarplot)
```
...
num_visits =  7 #length(unique(rs_1005$VISITNUM))
...
par(mar = c(5, 4, 4, 5) + 0.1)
# PC1s
plot(1:num_visits, PC1s_f[1, ], type = "l", col = 1, xlab = "Visit", ylab = "PC1", xlim = c(1, num_visits), ylim = range(PC1s_f), main = "PC1 Weights (Unscaled)")
for (row in 2:num_features) {
...
par(mar = c(5, 4, 4, 5) + 0.1)
plot(1:num_visits, PC2s_f[1, ], type = "l", col = 1, xlab = "Visit", ylab = "PC2", xlim = c(1, num_visits), ylim = range(PC2s_f), main = "PC2 Weights (Unscaled)")
...
hist(bootstrap_results$t, breaks = 30, main = "Histogram of Phi Parameters",
     xlab = "Phi Parameter")
mean_phi <- mean(bootstrap_results$t)
print(mean_phi)
```
...
treatment_effect_fn <- function(data, num_bootstraps){
  ...
  for (i in 1:num_bootstraps){
    ...
    num_patients <- length(unique(sampled_data$USUBJID))
    # randomly select half of the bootstrap sample
    selected_patients <- sample(unique(rs_1005_nsaa$USUBJID),
                                size = num_patients / 2)
    # Define the treatment effect
    treatment_effect <- 5
    # record treatment
    sampled_data$TREAT <- ifelse(sampled_data$USUBJID %in% selected_patients, 1, 0)
    # simple translation to the scores
    sampled_data$RSORRES <- ifelse(sampled_data$USUBJID %in% selected_patients,
                                 sampled_data$RSORRES + treatment_effect,
                                 sampled_data$RSORRES)
    ...
    # We assume that with treatment the NSAA mean is larger hence lower.tail=FALSE
...
set.seed(123)
bootstrap_p_values <- treatment_effect_fn(rs_1005_nsaa, num_bootstraps = 500)
...
# Simulate data for N trajectories = 10 * n patients
...
print(summary_gls)
```
Yet to do bootstrap analysis on how the size of the sample affects the observed significance of the treatment effect. But so far from experimenting with the treatment effect and size, a size of 100 patients (50 treated and 50 untreated) should be enough to distinguish between treated and untreated groups because the model can detect a significant treatment coefficient even when the true treatment effect is 0.5!


# TO DO LIST

1.) choose half of the patients and alter their trajectory to mimic a treatment effect- add a column for treatment, fit a gls model and see if you can identify whether someone got treated. Ultimately, you want to investigate how the size of the treatment effect affects the coefficient. Then bootstrap. To see how the sample size effects how easy it is to visualize the difference between treated and untreated patients.

2.) Plot weights when scale=FALSE

3.) Plot variances of each component with time
