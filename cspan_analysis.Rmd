...
# 1.) perform bootstrap on the gls model to see the histogram of values that the phi parameter takes, but also to investigate whether the correlation between the random effects and the error in the model is significant - gls assumes that they can be correlated which isn't 
...
# I am envisioning a matrix which contains the trajectories so the values across time for each USUBJID (for a specific RSTEST) and for NA values we assing the mean of the column) <- THIS MAY NOT EVEN BE REQUIRED


# 3.) Add grey bar to the bottom of the plot to show how many individuals at each time point

# 4.) Use the Kalman filter to perform dynamic PCA - we have weights from PCA at visit 1 then we update them for visit 2 based on the information from PCA
```
...
rs_1005_nsaa <- rs_1005_nsaa %>%
  mutate(VISIT = case_when(
    ...
    TRUE ~ NA_real_ # This line handles any VISITNUM values outside 0-9
  ))
...
# plots the residuals for every visit
plot(nsaa_total_gls, resid(., type="p")~fitted(.)|VISIT, abline=0)
# get residuals
# check autocorrelation within residuals (get autocorrelation by subject)
# check if there is clustering within the residuals of the gls - it might give some indication of grouping / clustering
# we assume no grouping when using this model - maybe if there is clustering in the residuals we might need to use a mixed model - also use LCTM to justify this if necessary
# need to make sure that the epsilon and random effects are uncorrelated - enforce this - research why this is necessary or something we want
# bootstrap fitting this model and extract the visit correlation coefficient and plot a histogram to see its distribution - if it is centered around zero then we need to remove it. Also get the phi parameter and see its distribution.
# fit the model without ar1 correlation - then plot the residuals and check its autocorrelation using a lag (t+1 vs t) plot.
```

Let's interpret the standardised residuals vs the fitted values graph - it illustrates the grouping structure by visit very clearly. Fitted values in this sense is the NSAA total score - so time essentially is moving in the reverse direction to the fitted values such that the last vertical line of points represents the baseline measurement. The standardized residuals follow a standard normal distribution for each visit which may indicate no obvious grouping structure of subjects through time. For an obvious grouping structure we would've expected certain residual points being clustered together at each visit point, but this is not completely obvious.

The negative correlation for VISIT in the context of the model indicates that as time progresses (as indicated by the VISIT variable), the response variable (RSORRES) is expected to decrease, adjusting for the time modeled. This adjustment means that the model accounts for changes over time in RSORRES, predicting its value based on the passage of time (represented by VISIT), while controlling for individual variability across subjects (USUBJID). The model suggests that each additional unit increase in VISIT is associated with a decrease in the expected value of RSORRES, after accounting for the autocorrelation in the data as indicated by the AR(1) structure.


The summary indicates an AR(1) correlation structure in your GLS model, with a high phi estimate of 0.9639783, suggesting strong correlation between the nsaa score of a single patient between two visits (i.e. 12 weeks apart).