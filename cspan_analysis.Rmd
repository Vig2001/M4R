...
```{r}
# Initialize new_trajectories matrix with zeros
new_trajectories <- matrix(0, 131, 6)  # Assuming 131 is the total number of patients initially

# Initialize a vector to keep track of all patient IDs
patient_ids <- seq_len(131)  # Assuming patient IDs go from 1 to 131

# Initialize a logical vector to track which patients are still in the study
...
for (i in 0:5) {
  ...
  if (num_active < length(active_patients)) {
    ...
    # This is a simplification; specific logic will depend on how patients drop out
    active_patients[(num_active + 1):length(active_patients)] <- FALSE
  }
  
  # Find the indices of active patients
  active_indices <- which(active_patients)
  
  # Calculate the product of X and the first column of the loadings matrix
  result_vector <- X %*% loadings_matrix[ ,1]
  
  # Place the result in the correct positions in new_trajectories
  new_trajectories[active_indices, i + 1] <- result_vector
  
  # Assume the bottom-most rows in X are from patients who are still in the study
  # (Adjust according to actual data scenario)
}

# Assuming new_trajectories is already filled as per previous steps
...
# Convert Visit to a numeric type for plotting (assuming visits are labeled V1, V2, ..., V6)
...
plot <- ggplot(trajectory_long_df, aes(x = Visit, y = Trajectory, group = PatientID)) +
  geom_line(color = "grey", size = 1) +  # Change line color to grey
  geom_point(aes(color = Dropout), size = 2, shape = 21, fill=NA) +  # Adjust point size and style
  scale_color_manual(values = c("Active" = "blue", "Dropout" = "red")) +
...
# Calculate the mean and standard deviation of the phi values in the data
phi_bar <- mean(phi_data$Phi)
sd_phi <- sd(phi_data$Phi)

# Plot histogram with Gaussian density overlay using the mean of phi_data
phi_histogram <- ggplot(phi_data, aes(x = Phi)) + 
  geom_histogram(aes(y = ..density..), bins = 30, fill = "lightblue", color = "black") +
  stat_function(fun = dnorm, args = list(mean = phi_bar, sd = sd_phi),
                aes(color = "Gaussian Density"), size = 1) + # Assign a color aesthetic to create a legend
...
# Loop over sample sizes and treatment effects
set.seed(123)
