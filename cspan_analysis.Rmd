...
cov_matrix <- cov(X0)# <- some evidence for scale=TRUE (variances of each component do be different).
selected_items <- c(1, 10,16, 17)
cov_subset <- cov_matrix[selected_items, selected_items]
latex_table <- xtable(cov_subset)
print(latex_table)
print(diag(cov_matrix))
```
...
test_components <- c("NSAA1-Stand", "NSAA1-Walk", "NSAA1-Rise From Floor", "NSAA1-Run (10m)")

# Generate plots
plots <- lapply(test_components, plot_ternary)

grid_plot <- grid.arrange(grobs = plots, ncol = 2, nrow = 2,
                          heights=c(10, 10), widths=c(10, 10))
```
