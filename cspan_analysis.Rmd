...
lctm_1 <- hlme(fixed = RSORRES ~ VISIT + VISIT**2, 
               subject = 'USUBJID_numeric',
               ng = 1,
               data = rs_1005_nsaa)
lctm_2 <- gridsearch(hlme(fixed = RSORRES ~ VISIT + VISIT**2,
                          mixture =~VISIT + VISIT**2,
                          ng = 2,
                          subject = 'USUBJID_numeric',
                          data = rs_1005_nsaa),
                     rep=100, maxiter=30, minit=lctm_1)
lctm_3 <- gridsearch(hlme(fixed = RSORRES ~ VISIT + VISIT**2,
                          mixture =~VISIT + VISIT**2,
                          ng = 3,
                          subject = 'USUBJID_numeric',
                          data = rs_1005_nsaa),
                     rep=100, maxiter=30, minit=lctm_1)
lctm_4 <- gridsearch(hlme(fixed = RSORRES ~ VISIT + VISIT**2,
                          mixture =~VISIT + VISIT**2,
                          ng = 4,
                          subject = 'USUBJID_numeric',
                          data = rs_1005_nsaa),
...
treatment_effect_fn <- function(data, num_bootstraps){
  ...
  for (i in 1:num_bootstraps){
    ...
    gls_model <- gls(RSORRES ~ 1 + VISIT + VISIT**2 + TREAT, 
                          data = sampled_data, 
                          correlation = corAR1(form = ~ 1 | USUBJID))
...
# Parameters
N <- 1000 # Number of trajectories - 100 patients
visits <- 1:10 # Visits from 1 to 10
treatment_effect <- 0.4# significant treatment effect
