...
# 1.) use nlme gls to estimate the correlation parameter - no need to worry about mean because we can add this on later - it doesn't affect correlation.

# model would be RSORRES ~ 1 + (time in weeks - check for each visitnum) + (time in weeks) ** 2 + AR(1) correlation structure
...
# 4.) Add grey bar to the bottom of the plot to show how many individuals at each time point
# or do Kalman filter to perform dynamic PCA - we have weights from PCA at visit 1 then we update them for visit 2 based on the information given
```
