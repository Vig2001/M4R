...
# 1.) choose half of the patients and alter their trajectory to mimic a treatment effect- add a column for treatment, fit a gls model and see if you can identify whether someone got treated. Ultimately, you want to investigate how the size of the treatment effect affects the coefficient. Then bootstrap.

# to see how the sample size effects how easy it is to visualize the difference between treated and untreated patients.
...
```{r}
num_patients <- length(unique(rs_1005_nsaa$USUBJID))

# Randomly select half of the patients
set.seed(123) # For reproducibility
selected_patients <- sample(unique(rs_1005_nsaa$USUBJID), size = num_patients / 2)

# Define the treatment effect
treatment_effect <- 0.63 # Modify - this gives p-value of 0.0499
rs_1005_treatment <- rs_1005_nsaa
# record treatment
rs_1005_treatment$TREAT <- ifelse(rs_1005_treatment$USUBJID %in% selected_patients, 1, 0)
# simple translation to the scores
rs_1005_treatment$RSORRES <- ifelse(rs_1005_treatment$USUBJID %in% selected_patients,
                               rs_1005_treatment$RSORRES + treatment_effect,
                               rs_1005_treatment$RSORRES)
nsaa_treatment_gls <- gls(RSORRES ~ 1 + VISIT + VISIT**2 + TREAT, 
                        data = rs_1005_treatment, 
                        correlation = corAR1(form = ~ 1 | USUBJID))
summary(nsaa_treatment_gls)
```