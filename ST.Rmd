...
1. Create the mean trajectory of NSAA scores, by inspecting the graph given in the Natural History Paper

2. Add in a random error so that we get rough individual trajectories.

3. Accounted for correlation between errors at different time points for the same subject by using an AR(1) process.

4. add treatment effect
...
generateValues <- function() {
  m_values <- m(1:16)
  rnorm_values <- rnorm(16, mean = 0, sd = 2) # Adjust the standard deviation (sd) here
...
phi_error <- 0.5  # Autoregressive coefficient for the error
...
plot(1:16, NSAA[[1]], type = 'l', xlab = 'Age', ylab = 'NSAA', main = 'Trajectories')

# Loop through the remaining trajectories and add them to the existing plot
for (i in 2:10) {
  lines(1:16, NSAA[[i]], type = 'l', col = i)  # Use different colors for each trajectory
}
