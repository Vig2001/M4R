...
Y_pca <- prcomp(Y)
screeplot(Y_pca)
Y_pca$sdev[1]^2/sum(Y_pca$sdev^2)
```
...
simulate_scores <- function(latent_trait, n_items) {
  ...
  for (i in 1:n_items) {
    discrimination <- runif(1, 0.5, 2.0) # alter discrimination param
    thresholds <- sort(runif(2, -2, 2)) # introduce thresholds for latent trait
