...
It is clear that to capture a large proportion of the variance we need to incorporate a multi-dimensional scoring system i.e. we have to instruct doctors to perform some form of matrix multiplication at each visit! This lacks interpretability and may not generalise well to new data.

## Temporal Analysis

Here we will in the datasets of the remaining visits which aren't severely affected by dropout (visit 1 - visit 5). ChatGPT made the process dynamic.
...
transformed_totals <- function(Visit_mat){
  ...
  pca_res <- PCA(Visit1_mat, graph = F, scale.unit = T)
  eig_df <- as.data.frame(get_eigenvalue(pca_res))
  cum_var <- eig_df$cumulative.variance.percent
  # use 75% as cut off for now
  ndim <- which.min(abs(cum_var - 75))
  if (cum_var[ndim] < 75){
    ndim <- ndim + 1
  }
  # set ncp in new PCA
  pca_new <- PCA(Visit1_mat, graph = F, scale.unit = T, ncp = ndim)
  loadings <- pca_new$var$cor
  # get transformed scores
  scores <- Visit1_mat %*% loadings
