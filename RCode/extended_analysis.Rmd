...
transformed_totals <- function(Visit_mat){
  ...
  pca_new <- PCA(Visit_mat, graph = F, scale.unit = T, ncp = 2)
...
The phi parameter gives us a good indication of interpretability, a higher $\phi$ indicates a larger correlation between consecutive timepoints which aligns with the hypothesis that a patient's score strongly correlates with their most previous score. When we use 1 PC, we get a phi of 0.08 essentially indicating a random scoring system - this probably won't generalise well. Moreover, as we increase the number of dimensions we get a negative phi which essentially indicates that a patient's score will be the opposite of their most recent score. This doesn't make sense - it indicates volatility! Ask Chris.