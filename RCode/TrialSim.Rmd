...
Below we do not resimulate the baseline scores everytime, which means we can use the decay rates from decay_rates_df. We also bypass issue of potentially different baseline columns (recall issue we had with only 50 columns being visualised at baseline instead of 51).
```{r}
num_boots <- 50
teffs <- seq(1.1, 2, length.out=5)
...
# timing NSAA, PCA and IRT
method_times <- numeric(length(scoring_methods))
names(method_times) <- scoring_methods
  ...
  for (method in scoring_methods){
    ...
    for (i in 1:num_boots){
      ...
      ctrlsample_end <- as.data.frame(apply(ctrlsample_base, 2, function(col){
        sapply(col, 
               function(state) simulate_decay(state, 
                                              decay_rates_df[col, 1],
                                              decay_rates_df[col, 2],
      ...
      # 7. calculate power
      coefs <- bootlm_summary$coefficients
      p_val <- coefs["Group1:Time1", "Pr(>|t|)"]
      power_iter[i] <- as.numeric(p_val < 0.05) # 1 if so else 0
    }
    
    power_mat[teff_idx, method] <- mean(power_iter) # proportion of 1s = power
  }
}
```
...
Note this is only for 50 bootstraps to get more reliable results need to do 1000 but will need HPC. Parallel computing for speed - ask Chris to see if he knows how to do this / can do it on Imperial HPC.