...
scoring_method <- function(score, base = TRUE, ctrl = TRUE) {
  ...
  do_pca <- function(base, ctrl) {
    ...
    # Select the subset (base or end) and compute scores
...
grm <- mirt(encode(combined_ctrl), 1, itemtype = "graded")
weights <- coef(grm, IRTpars = TRUE)

# all 52 encoded item names
item_names <- colnames(encode(combined_ctrl))
irt_weights <- sapply(item_names, function(item) weights[[item]][, "a"])

ctrl_irt_base <- as.matrix(encode(ctrl_grp_base)) %*% irt_weights[colnames(encode(ctrl_grp_base))]
```
